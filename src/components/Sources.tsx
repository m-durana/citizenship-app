...
function CountryBlock({ group }: { group: CountryGroup }) {
  return (
    <section>
      <h2 className="text-2xl font-bold tracking-[-0.02em] mb-4 flex items-center gap-3">
        ...
        {group.paths.map((p) => (
          <div key={p.id}>
            <div className="flex items-baseline gap-2 flex-wrap mb-2">
              <h3 className="font-medium text-ink/90">{p.name}</h3>
              <span className="text-[10px] font-mono uppercase tracking-[0.18em] text-muted border border-border px-1.5 py-0.5">
